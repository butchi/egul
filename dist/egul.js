...
var Egul = function () {
  function Egul() {
    ...
    this.$codeFrame = (0, _jquery2.default)('<div class="code-frame"></div>');
    this.$codeLang = (0, _jquery2.default)('<div class="code-lang"></div>');
    this.$highlight = (0, _jquery2.default)('<div class="highlight"></div>');
    this.$preCode = (0, _jquery2.default)('<pre><code></code></pre>');
  }

  _createClass(Egul, [{
    key: 'renderHtml',
    value: function renderHtml() {
      var htmlTxt = _.template('<html>\n' + '  <head>' + '<%= head %>\n' + '  </head>\n' + '  <body>' + '<%= body %>\n' + '  </body>\n' + '</html>')({
      ...
      (0, _jquery2.default)('.block-source--html').append($frame);
    }
  }, {
    key: 'renderAssets',
    value: function renderAssets(fileArr) {
      ...
      var $blockSrc = (0, _jquery2.default)('.block-source--list');
      fileArr.forEach(function (fileName) {
        _jquery2.default.ajax({
          ...
          success: function success(res) {
            ...
            var $frame = _this.$codeFrame.clone();
            var $hl = _this.$highlight.clone();
            var $lang = _this.$codeLang.clone().text(fileName);
            var $code = _this.$preCode.clone().text(srcTxt);
            $hl.append($code);
            $frame.append($lang).append($hl);
